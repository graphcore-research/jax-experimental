...
def transformer_block(x, mlp, attn, ln_1, ln_2, n_head):  # [n_seq, n_embd] -> [n_seq, n_embd]
    ...
    x = x + mha(layer_norm(x, **ln_1), **attn, n_head=12)  # [n_seq, n_embd] -> [n_seq, n_embd]
...
def gpt_basic(inputs, wte, wpe, w, b):
    x = wte[inputs, :] 
    # x+= wpe[np.arange((len(inputs)))]  # [n_seq] -> [n_seq, n_embd]
    x+= wpe[np.array([a for a in range((len(inputs)))])]  # [n_seq] -> [n_seq, n_embd]
    x = linear(x, w, b)
    out = x @ wte.T
    return out
...
def gpt2(inputs, wte, wpe, blocks, ln_f, n_head):  # [n_seq] -> [n_seq, n_vocab]
    ...
    # for block in blocks:
    # x = transformer_block(x, **blocks[0], n_head=n_head)  # [n_seq, n_embd] -> [n_seq, n_embd]
...
# @partial(jax.jit, static_argnames=["tokens_to_generate"])
# @partial(jax.jit, static_argnames=["n_head", "tokens_to_generate"])

@jax.jit
def generate(inputs, params, n_tokens_to_generate):
    # def generate(inputs, wte, wpe, w, b, n_tokens_to_generate):
    start = 5
    for _ in tqdm(n_tokens_to_generate):  # auto-regressive decode loop
        # import ipdb; ipdb.set_trace()

        # import ipdb; ipdb.set_trace()
        logits = gpt2(inputs, params["wte"], params["wpe"], params["blocks"], params["ln_f"], n_head=12)  # model forward pass
        # logits = gpt2(inputs, **params, n_head=n_head)  # model forward pass
        # logits = gpt_basic(inputs, wte, wpe, w, b)  # model forward pass
        # print(logits[-1], logits[-1].shape, np.argmax(logits[-1]))
        # print(logits.shape, logits[-1][0])
        next_id = np.argmax(logits[-1])  # greedy sampling
        # mask = mask.at[start + i].set(1)
        # import ipdb; ipdb.set_trace()
        # inputs = inputs.at[start + _].set(next_id)  # append prediction to input
        # import ipdb; ipdb.set_trace()
...
def main(prompt: str, n_tokens_to_generate: int = 15, model_size: str = "124M", models_dir: str = "models"):
    ...
    wte = np.array(onp.random.random((len(input_ids), 100)))
    wpe = np.array(onp.random.random((len(input_ids), 100)))
    w = np.array(onp.random.random((100, 100)))
    b = np.array(onp.random.random((100)))
    # output_ids = generate(input_ids, wte, wpe, w, b, n_tokens_to_generate)
    output_ids = generate(input_ids, params, n_tokens_to_generate)
